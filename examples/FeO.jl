...
Ecut = 15
kgrid=[2,2,2]
model = model_LDA(lattice, atoms, positions; temperature = 0.005, smearing = Smearing.MarzariVanderbilt(), symmetries = true, magnetic_moments)
fft_size = compute_fft_size(model, Ecut, kgrid; supersampling=2*sqrt(2))
basis = PlaneWaveBasis(model; Ecut=15, kgrid=[2,2,2], fft_size)

...
def read_data(filename):
    ...
    for line in lines[1:]:
        ...
        x_values.append(numbers[0]-6.034) #6.2285)
...
def plot_multiple_files(filenames, output_filename, vline_x=None):
    ...
    for i, filename in enumerate(filenames):
        ...
        plt.plot(x_values, y_values, label=f'File {i + 1}, Series 1 (Integral: {integral:.2f})')
...
filenames = ['Si.pdos_atm#1(Si)_wfc#1(s)', 'Si.pdos_atm#1(Si)_wfc#2(p)', 'Si.pdos_atm#2(Si)_wfc#1(s)', 'Si.pdos_atm#2(Si)_wfc#2(p)']
...
plot_multiple_files(filenames, 'plot_Si_pdos.png', vline_x=vertical_line_x)